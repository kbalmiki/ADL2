...
# modifying (removed final sigmoid)
    def forward(self, x):
        h = F.relu(self.fc1(x))
        out = self.fc2(h)  # no sigmoid!
        return out
...
# Loss function for task 1.2
def logistic_loss(preds, targets):
    # preds are logits, not passed through sigmoid
...
# Logistic loss (no sigmoid in discriminator)
